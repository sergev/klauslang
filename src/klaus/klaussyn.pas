...
// 3. Правила со знаком ">>" должны во всех родительских группах "ИЛИ" быть
// расположены прежде, чем "похожие" правила без этого знака. Иначе "похожие"
...
const
  // Имя корневого синтаксического правила для исходного файла
  klausSourceSyntaxRoot = 'klaus_source_root';

  // Имя корневого синтаксического правила для вычисления выражения
  ...
  klausSynRules: array of tKlausSynRuleRec = (
    ...
    (name: 'program';
    def:   '`программа` #id [<program_params>] ";" [<uses_list>] <routine> "."'),

    (name: 'program_params';
    def:   '"(" [`вх`] #id ":" `массив` `строк` ")"'),

    (name: 'uses_list';
    def:   '`используется` <uses> [* "," <uses>] ";"'),
